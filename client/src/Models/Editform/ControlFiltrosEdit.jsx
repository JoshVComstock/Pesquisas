...
const ControlFiltrosEdit = ({ controlfiltroactual, MostrarControlFiltros }) => {
  const [fecha, setFecha] = useState(controlfiltroactual.fecha);
  const [numero_correlativo, setNumCorrelativo] = useState(controlfiltroactual.numero_correlativo);
  const [cod_tarjeta_inicial, setCodInicial] = useState(controlfiltroactual.cod_tarjeta_inicial);
  const [cod_tarjeta_final, setCodFinal] = useState(controlfiltroactual.cod_tarjeta_final);
  const [cantidad, setCantidad] = useState(controlfiltroactual.cantidad);
  const [id_centros, setId_Centros] = useState(controlfiltroactual.id_centros);
  const [entregado_por, setEntregado_por] = useState(controlfiltroactual.entregado_por);
  const [recibido_por, setRecibido_por] = useState(controlfiltroactual.recibido_por);
  const [centros, setCentros] = useState([]);

  const Editar = async (e) => {
    e.preventDefault();
    const response = await fetch("http://127.0.0.1:8000/api/control_filtros/" + controlfiltroactual.id, {
      method: "PUT",
      headers: {
...
export default ControlFiltrosEdit
